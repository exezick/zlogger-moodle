...
namespace logstore_zlogger\task;
...
class cleanup_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('taskcleanup', 'logstore_zlogger');
    }
    ...
    public function execute() {
        ...
        $loglifetime = (int)get_config('logstore_zlogger', 'loglifetime');
        ...
        while ($min = $DB->get_field_select("logstore_zlogger_log", "MIN(timecreated)", "timecreated < ?", $lifetimep)) {
            ...
            $DB->delete_records_select("logstore_zlogger_log", "timecreated < ?", $params);
            if (time() > $start + 300) {
        ...
        mtrace(" Deleted old log records from zlogger store.");
    }
}
